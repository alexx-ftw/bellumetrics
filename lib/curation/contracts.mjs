...
export function parseAgentDecisionForCase(value, originalCase) {
  ...
  if (mutation.participants) {
    const firstSide = commanders[0].side;
    if (typeof firstSide !== "string" || !firstSide.trim() || firstSide !== firstSide.trim()
      || mutation.participants.length <= commanders.length
      || commanders.some((commander, i) => commander.side !== firstSide
        || mutation.participants[i].ref.id !== `war-atlas:${commander.slug}`
        || mutation.participants[i].side !== commander.side)
      || mutation.participants.slice(commanders.length).some(p => p.side === firstSide)) {
      fail("participants cannot overwrite original commanders; append only the missing opposing side");
    }
    return decision;
  }
