...
For a nonempty list with exactly one distinct nonempty side, investigate the missing opposing military command before escalating. A reviewed participants proposal may retain every original commander as an exact ordered prefix (same war-atlas:source_slug and side) and append verified opponents. Never change existing identifiers, order, or sides. Every appended participant must belong to the absent opposing side. If an original side or identity is ambiguous, escalate.
...
When the original commanders list is empty or contains only one fully identified side, investigate the actual military command and side membership before escalating. An approval may include canonicalMutation.participants: [{"ref":{"type":"commander","id":"real supplied or verified reference"},"side":"verified side"}, ...]. It must align exactly with commanderRefs, have distinct commanders and exactly two distinct sides. Use only identities verified in canonical commanders or staged War Atlas commander records; never fabricate a source slug. Evidence must support each participant's command role and side, not merely their political office. If these identities cannot be verified with available tools, escalate explicitly.

For an empty original list, order participants by exact reference ID and interpret outcome from its first distinct side. For a one-sided original list, retain the complete original ordered prefix, append opponents sorted by exact reference ID, and interpret outcome from the original first side. Both roles must agree on the entire proposal. Existing two-sided lists must use participants: null. The parser omits null before persistence. All added identities must be verified in canonical commanders or staged War Atlas commander records, with cited evidence of command and side. Raw imported data is never changed.
